...
- Trang bệnh nhân dùng cùng sidebar và class `role-page web-dashboard patient-theme`. Giao diện các trang Bệnh nhân hiện tại đã được nâng cấp thiết kế chuẩn UI/UX rất đẹp (Metric Cards, SVG Icons, bo góc 16px, bóng đổ shadow-sm, giao diện Ticket). Mọi màn hình mới thêm vào hoặc chỉnh sửa bắt buộc phải bám sát phong cách thiết kế hiện đại này.
